...
- 마지막으로 역전파는 순전파의 역순으로 각 계층의 backward 메서드를 호출하면 됩니다. 은닉층의 뉴런은 순전파 시에 여러 개로 복사되었는데 이는 '1.3.4 계산 그래프'에서 설명한 Repeat 노드에 해당합니다. 따라서 역전파 때는 여러 기울기 값을 더해줍니다. 이상으로 네거티브 샘플링을 모두 구현했습니다.

## 4.3 개선판 word2vec 학습

- 지금까지 word2vec을 개선하기 위해 Embedding 계층과 네거티브 샘플링 기법을 설명하고 이를 구현했습니다. 그럼 이제 이를 신경망 구현에 적용해 PTB 데이터셋을 사용해 학습하고 더 실용적인 단어의 분산 표현을 얻어보겠습니다.

### 4.3.1 CBOW 모델 구현

- 이번에 구현할 CBOW는 앞 장의 SimpleCBOW를 Embedding 계층과 Negative Sampling Loss 계층을 적용해 개선할 것입니다. 여기에 맥락의 윈도우 크기를 조절할 수 있도록 하겠습니다.

> 자세한 내용은 4.3_cbow.py를 확인하세요.

- 초기화 메서드는 vocab_size(어휘 수), hidden_size(은닉층의 뉴런 수), corpus(단어 ID 목록), window_size(맥락의 크기)를 받습니다.

#### Warning

> SimpleCBOW는 입력 측 가중치와 출력 측 가중치의 형상이 달라 출력 측 가중치에서 단어 벡터가 열 방향이었습니다.
>
> 하지만 CBOW는 입력 측과 출력 측의 가중치 형상이 같아 단어 벡터가 행 방향으로 배치됩니다.
>
> 이는 NegativeSamplingLoss에서 Embedding 계층을 사용하기 때문입니다.

- 초기화가 끝나면 계층을 생성하는데 여기서는 Embedding 계층을 2 \* window_size개 생성해 in_layers 배열에 보관합니다. 그 다음 Negative Sampling Loss 계층을 생성합니다.

- 계층 생성이 끝나면 신경망의 모든 매개변수와 기울기를 params와 grads에 모으고 나중에 단어의 분산 표현에 접근할 수 있도록 word_vec에 가중치 W_in을 할당합니다.

- 이어서 순전파와 역전파는 각 계층의 순전파와 역전파를 적절한 순서로 호출하여 앞 장의 SimpleCBOW를 확장합니다.

  - 단, forward 메서드의 인수로 받는 맥락과 타깃이 단어 ID라는 점이 다를 뿐입니다. 구체적인 예는 다음과 같습니다.

  <img src="README.assets/fig 4-19.png" alt="fig 4-19" style="zoom:50%;" />

  - 위 그림의 단어 ID 배열이 contexts와 target의 예시로 이러한 데이터가 forward의 인수로 입력됩니다.

- 이상으로 CBOW의 구현이 완료되었습니다.

### 4.3.2 CBOW 모델 학습

- 마지막으로 CBOW 모델을 학습하겠습니다. 여기에선 단순히 신경망 학습을 수행합니다.

> 자세한 내용은 4.3.2_train.py를 확인하세요.

- 이번 장의 CBOW 모델은 윈도우 크기를 5로, 은닉층의 뉴런 수를 100개로 설정하였으나 이는 말뭉치에 따라 다릅니다.

  - 다만, 윈도우 크기는 2 ~ 10, 은닉층의 뉴런 수는 50 ~ 500 사이에서 좋은 결과를 얻습니다.

- 그러나 이번에 다루는 PTB는 지금까지 사용한 말뭉치보다 월등히 커 학습 시간이 상당히 오래 걸립니다. 그래서 해당 파일에 GPU를 사용할 수 있습니다.

  - 단, GPU를 사용하기 위해서 nvidia의 GPU와 cupy의 설치가 필요합니다.

- 학습이 종료되면 가중치(입력 측)를 꺼내 나중에 이용할 수 있도록 보관합니다. 이 때 Python의 pickle을 사용하여 객체를 파일로 저장합니다.

#### Warning

> 학습이 완료된 매개변수는 cbow_params.pkl에 준비되어 있습니다.
>
> 학습 종료를 기다리기 힘들다면 해당 매개변수를 이용하세요.
>
> 참고로 학습된 가중치는 각자 학습한 환경에 따라 다른데 이는 학습 과정에서 무작위 값이 사용되기 때문입니다.
>
> 따라서 최종적으로 얻는 가중치가 환경에 따라 달라지지만 거시적으로 비슷한 결과를 얻을 수 있습니다.

### 4.3.3 CBOW 모델 평가

- 그럼 CBOW 모델이 학습한 단어의 분산 표현을 평가하겠습니다. 2장에서 구현한 most_similar 메서드를 이용할 것이며 각 단어의 거리가 가장 가까운 단어를 찾아보겠습니다.

> 자세한 내용은 4.3.3_eval.py를 확인하세요.

- 이 코드의 결과를 분석하면 다음과 같습니다.

  - you의 경우 비슷한 인칭대명사인 i와 we 등이 나옵니다.

  - year의 경우 month와 week같은 기간을 뜻하는 단어들이 나옵니다.

  - toyota의 경우 ford, mazda, nissan과 같은 자동차 메이커가 나옵니다.

- 이를 통해 CBOW 모델의 단어 분산 표현은 괜찮은 특성을 지닌다고 할 수 있습니다.

- word2vec으로 얻은 단어의 분산 표현은 미슷한 단어를 가까이 모을 뿐 아니라 더 복잡한 패턴을 파악할 수 있습니다.

  - 대표적으로 'king - man + woman = queen'과 같은 유추 혹은 비유 문제입니다.

  - 더 정확하게 word2vec의 단어 분산 표현을 사용해 유추 문제를 덧셈과 뺄셈을 사용해 풀 수 있습니다.

    - 실제로 유추 문제를 풀려면 단어 벡터 공간에서 'man - woman' 벡터와 'king - ?'가 가까워지는 단어를 찾습니다.

    <img src="README.assets/fig 4-20.png" alt="fig 4-20" style="zoom:50%;" />

- 단어 man의 분산 표현을 vec(man)이라고 한다면 얻고 싶은 관계는 'vec(woman) - vec(man) = vec(?) - vec(king)'입니다. 즉, 해당 벡터에 가장 가까운 단어 벡터를 구하게 됩니다.

  - 이 방법을 구현한 파일이 commons/util.py의 anology 메서드로 이와 같은 유추 문제를 한 번에 처리할 수 있습니다.

  > 자세한 내용은 commons/util.py의 anology와 4.3.3_eval.py를 확인하세요.

- anology 메서드를 몇 개의 예제를 가지고 확인하면 다음과 같습니다.

  - king : man = queen : ?의 경우 woman을 파악했습니다.

  - take : took = go : ?의 경우 went로 답하며 현재형과 과거형의 패턴을 파악하고 있음을 알 수 있습니다.

  - car : cars = child : ?의 경우 children을 답하며 단수형과 복수형을 구분할 수 있습니다.

  - 하지만 good : better = bad : ?의 경우 worse가 아닌 다른 답이 나오는데 more과 less 같은 단어가 나오는 걸 보아 비교급에 대해서도 인코딩되어 있음을 알 수 있습니다.

- 이처럼 word2vec으로 얻은 단어의 분산 표현을 사용하면 벡터의 덧셈과 뺄셈으로 유추 문제를 해결할 수 있습니다. 즉, 단어의 단순 의미뿐 아니라 문법적 패턴도 파악할 수 있는 것입니다.

#### Warning

> 앞서 본 유추 문제의 결과는 매우 뛰어나 보였지만 사실 원하는 결과를 얻지 못하는 문제가 더 많을 것입니다.
>
> 이는 PTB 데이터셋이 작기 때문으로 더 큰 말뭉치로 학습하면 더 정확하고 견고한 단어의 분산 표현을 얻을 수 있습니다.
>
> 따라서 그런 경우 유추 문제의 정답률도 크게 향상될 것입니다.

## 4.4 word2vec 남은 주제

- word2vec의 구조와 구현에 대해 설명을 마무리하기 전에 이번 절에서 word2vec에 관해 지금까지 다루지 못한 주제를 소개하겠습니다.

### 4.4.1 word2vec을 사용한 애플리케이션의 예

- word2vec으로 얻은 단어의 분산 표현은 비슷한 단어를 찾는 용도로 이용할 수 있으나 분산 표현의 장점은 여기서 끝이 아니라 자연어 처리 분야에서 단어의 분산 표현을 **전이 학습**에 사용할 수 있다는 점입니다.

- 전이 학습은 한 분야에서 배운 지식을 다른 분야에도 적용하는 기법입니다.

  - 자연어 문제를 해결할 때 word2vec의 단어 분산 표현을 처음부터 학습하는 일은 거의 없고 학습이 끝난 데이터를 각자 작업에 사용합니다.

  - 즉, 텍스트 분류, 문서 클러스터링, 품사 태그 달기, 감정 분석 등 자연어 처리 작업에 필수적인 단어를 벡터로 변환하는 작업을 학습을 미리 끝낸 단어의 분산 표현을 사용하면 됩니다.

- 단어의 분산 표현은 단어를 고정 길이 벡터로 변환해준다는 장점도 있고 문장도 단어의 분산 표현을 사용해 고정 길이 벡터로 변환할 수 있다는 장점이 있습니다.

  - 가장 간단한 방법은 문장의 각 단어를 분산 표현으로 그 합을 구하는 bag-of-words라는 방식이 있으며 이는 단어의 순서를 고려하지 않는 모델입니다.

  - 또한 순환 신경망 RNN을 사용하면 한층 세련된 방법으로 문장을 고정 길이 벡터로 변환할 수 있습니다.

- 단어나 문장을 고정 길이 벡터로 변환할 수 있다는 점은 매우 중요한데 이는 일반적인 머신러닝 기법을 적용할 수 있기 때문입니다.

<img src="README.assets/fig 4-21.png" alt="fig 4-21" style="zoom:50%;" />

- 위의 그림처럼 자연어로 쓰여진 질문을 고정 길이 벡터로 변환할 수 있다면 그 벡터를 다른 머신러닝 시스템의 입력으로 사용할 수 있고 일반적인 머신러닝 시스템의 틀에서 원하는 답을 출력하거나 학습하는 것이 가능합니다.

#### Note

> 위 그림과 같은 파이프라인에서 단어의 분산 표현 학습과 머신러닝 시스템의 학습은 서로 다른 데이터셋을 사용해 개별적으로 수행하는 것이 일반적입니다.
>
> 예컨대 단어의 분산 표현은 위키백과와 같은 범용 말뭉치를 사용하고 현재 직면한 문제에 관한 데이터를 머신러닝 시스템에 사용합니다.
>
> 다만 직면한 문제의 데이터가 아주 많다면 단어의 분산 표현과 머신러닝 시스템 학습 모두 처음부터 수행할 수도 있습니다.

- 그럼 구체적인 예를 보면서 단어의 분산 표현 사용법을 살펴보겠습니다.

  - 수 많은 사람들이 사용하는 스마트폰 앱을 운용하는 회사에서 호의적인 의견, 불만 등이 메일이나 트윗 등으로 매일 쏟아져 들어온다고 가정합니다.

  - 이제 이들을 자동으로 분류하는 시스템을 만들텐데 메일의 내용을 보고 사용자의 감정을 정확하게 3단계로 분류하여 불만을 가진 사용자의 메일부터 순서대로 볼 계획입니다.

  <img src="README.assets/fig 4-22.png" alt="fig 4-22" style="zoom:50%;" />

  - 위와 같은 시스템을 위해서 우선 데이터를 수집하고 이 데이터들에 레이블을 붙입니다. 예를 들어 긍정적, 중립적, 부정적이 될 수 있을 것입니다.

  - 레이블링이 끝나면 학습된 word2vec을 이용해 메일을 벡터로 변환하고 감정 분석을 수행하는 분류 시스템에 벡터화된 메일과 감정 레이블을 입력해 학습합니다.

- 이처럼 자연어를 다루는 문제는 단어의 분산 표현이라는 방법으로 벡터화할 수 있고 그 덕분에 일반적인 머신러닝 기법으로 해결할 수 있습니다.

- 게다가 word2vec 전이 학습의 혜택을 누리는데 즉, 다양한 자연어 처리 문제에서 word2vec으로 구축한 단어의 분산 표현을 이용해 정확도를 높일 수 있습니다.

### 4.4.2 단어 벡터 평가 방법

- 이번 절에서는 word2vec을 통해 얻은 단어의 분산 표현이 얼마나 좋은지 검증하는 방법을 간략히 알아보겠습니다.

- 단어의 분산 표현은 현실적으로 특정 애플리케이션에서 사용되는 것이 대부분으로 궁극적으로 정확도 높은 시스템을 구축하려 합니다.

  - 여기서 생각할 점이 이런 시스템은 여러 시스템, 즉 단어의 분산 표현을 만드는 word2vec, 특정 문제를 분류하는 SVM이나 머신러닝 등으로 구성됩니다.

  - 더욱이 이 시스템들의 학습은 따로 수행하는 것이 가능합니다.

    - 예를 들어 분산 표현의 차원 수가 최종 정확도에 얼마나 영향을 주는지 조사하기 위해 단어의 분산 표현을 학습하고 그 분산 표현으로 머신러닝 시스템을 학습시킬 수 있습니다.

  - 즉, 두 단계의 학습을 수행한 다음 평가가 진행되는데 이 경우 시스템 각각 최적의 하이퍼파라미터를 찾기 위한 튜닝도 필요하므로 시간이 오래 걸립니다.

- 그래서 단어의 분산 표현의 우수성을 실제 애플리케이션과 분리해 평가하는 것이 일반적으로 이 때 자주 사용되는 척도가 단어의 '유사성'이나 '유추 문제'를 활용한 평가입니다.

- 단어의 유사성 평가는 사람이 작성한 단어 유사도를 검증 세트를 사용해 평가하는 것이 일반적으로 우선 사람이 단어 사이의 유사도를 규정합니다. 그리고 그 점수와 word2vec에 의한 코사인 유사도 점수를 비교해 그 상관성을 봅니다.

- 유추 문제를 활용한 평가는 'king : queen = man : ?'과 같은 문제를 출제하고 그 정답률로 단어의 분산 표현의 우수성을 측정하는 것입니다.

- 다음 표는 word2vec 모델, 단어의 분산 표현의 차원 수, 말뭉치의 크기를 매개변수로 비교 실험을 수행한 결과를 나타냅니다.

  - 여기서 '의미' 열은 단어의 의미를 유추하는 유추 문제의 정답률로 'king : queen = actor: actress'와 같은 문제입니다.

  - 반면 '구문' 열은 단어의 형태 정보를 붇는 문제의 정답룔로 'bad : worst = good : best'와 같은 문제입니다.

  <img src="README.assets/fig 4-23.png" alt="fig 4-23" style="zoom:50%;" />

#### Note

> 위 표의 결과에서 알 수 있는 것은 다음과 같습니다.
>
> 1. 모델에 따라 정확도가 다르므로 말뭉치에 따라 적합한 모델을 선택해야 합니다.
>
> 2. 일반적으로 말뭉치가 클수록 결과가 좋습니다.
>
> 3. 단어 벡터 차원 수는 적당한 크기가 좋습니다.

- 유추 문제를 이용하면 '단어의 의미나 문법적인 문제를 제대로 이해하고 있는지'를 측정할 수 있어 이를 정확하게 푸는 단어의 분산 표현은 자연어를 다루는 애플리케이션에서도 좋은 결과를 기대할 수 있습니다.

- 다만, 단어의 분산 표현의 우수성이 애플리케이션에 얼마나 기여하는지는 애플리케이션의 종류, 말뭉치의 내용 등 문제 상황에 따라 다르므로 이 점에 대한 주의가 필요합니다.

## 4.5 정리

- Embedding 계층은 단어의 분산 표현을 담고 있어 순전파 시 지정한 단어 ID의 벡터를 추출합니다.

- word2vec은 어위 수의 증가에 비례해 계산량이 증가하므로 근사치로 계산하는 빠른 기법을 사용하면 좋습니다.

- 네거티브 샘플링은 부정적인 예 몇 개를 샘플링하는 기법으로 이를 통해 다중 분류를 이진 분류처럼 취급할 수 있습니다.

- word2vec으로 얻은 단어의 분산 표현에는 단어의 의미가 녹아들어 있어 비슷한 맥락에서 사용되는 단어의 단어 벡터 공간에서 가까이에 위치합니다.

- word2vec의 단어의 분산 표현을 이용하면 유추 문제를 덧셈과 뺄셈으로 풀 수 있습니다.

- word2vec은 전이 학습 측면에서 특히 중요하며 그 단어의 분산 표현은 다양한 자연어 처리 작업에 이용할 수 있습니다.
