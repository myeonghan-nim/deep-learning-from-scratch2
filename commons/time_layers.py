...
class LSTM:
    def __init__(self, Wx, Wh, b):
        ...
        Wx: weight bias for x
        Wh: params for hidden h
        b: bias
        '''
        self.params = [Wx, Wh, b]
        self.grads = [np.zeros_like(Wx), np.zeros_like(Wh), np.zeros_like(b)]
...
class TimeSoftmaxWithLoss:
    ...
    def forward(self, xs, ts):
        ...
        if ts.ndim == 3:  # if answer in encoded one-hot
            ts = ts.argmax(axis=2)
        ...
        ls *= mask  # loss of ignore_label data is 0
        loss = -np.sum(ls)
        loss /= mask.sum()
    ...
    def backward(self, dout=1):
        ...
        dx *= dout
        dx /= mask.sum()
        dx *= mask[:, np.newaxis]  # grad of ignore_label datas is 0
...
# ====================================================================== #
# follow layers are not planned in book(simulate understand better not speed)
#
# TimeSigmoidWithLoss: time sigmoid + loss
# GRU: GRU
# TimeGRU: time GRU
# BiTimeLSTM: two way LSTM
# Simple_TimeSoftmaxWithLoss：simple TimeSoftmaxWithLoss
# Simple_TimeAffine: simple TimeAffine
# ====================================================================== #
